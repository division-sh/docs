...
<Steps>
  ...
  <Step title="Declare the events (events.yaml)">
    ...
    ticket.created:
      ...
      subject: text
      body: text
    ticket.classified:                # the classifier's verdict
      category: text
      priority: text
    ticket.assigned:                  # what the resolver needs to act
      category: text
      priority: text
    ticket.resolved:
      resolution: text
    ...
    field it declares; there are no defaults and no automatic copy from the triggering event.

    Notice each event carries only what its handler needs: `ticket.created` brings the raw text
  ...
  <Step title="Write the orchestrator (nodes.yaml)">
    ...
    ticket-orchestrator:
      ...
      event_handlers:
        ...
        ticket.classified:             # the classifier has labeled it
          ...
          emit:                        # hand off to the resolver, with a payload
            event: ticket.assigned
            fields:                    # fill every field ticket.assigned declares
              category: entity.category    # read from the ticket we just wrote
              priority: entity.priority

        ticket.escalated:              # the resolver bounced it back
          guard:
            id: escalation_limit
        ...
        ticket.resolved:               # the resolver succeeded
          ...
          emit:
            event: ticket.resolution_confirmed
            fields:
    ...
    **Filling the emitted payload.** When a handler emits an event that carries fields, it must
    populate every one: there are no defaults and nothing is copied automatically from the
    triggering event. That is why `ticket.classified` uses the object form of `emit`, with a
    ...
    wrote. (The bare string form is only for events that declare no payload fields.)
  ...
  <Step title="Add the agents (agents.yaml)">
    ...
    ```yaml agents.yaml
    classifier-agent:                 # map key matches the role in schema.yaml
  ...
  <Step title="Write the prompts (prompts/)">
    ...
    You resolve assigned tickets. Decide from the ticket category:

    - If the category is `billing` or `account`, the issue needs account access or identity
      checks, so emit `ticket.escalated` with a short reason.
    - Otherwise (`technical` or `feature`), emit `ticket.resolved` with a brief standard
      resolution for that category and your id in `resolved_by`.
    ```

    The resolver decides from `category` because that is what `ticket.assigned` carries. An
    ...
    the resolver works from the structured verdict.
  </Step>
...
With Postgres running (see [Installation](/installation)) and an LLM runtime configured (the
agents need one to answer), run the bundle. `swarm run` boots a runtime in process, publishes
the trigger, and streams the trace:

```bash
export SWARM_API_TOKEN=dev-token        # any value; required by every command but `swarm verify`

echo '{"ticket_id": "t-42", "subject": "Export button is greyed out", "body": "The CSV export button on the reports page is greyed out and I cannot click it."}' > ticket.json
...
<Note>
  ...
  is configured: a real provider (`SWARM_LLM_BACKEND=api` with `ANTHROPIC_API_KEY`) or the
  Claude CLI runtime (`SWARM_LLM_BACKEND=cli_test`). The deterministic system-node steps
  run either way; the agent steps wait until a runtime is present.
</Note>

## Watch one run

Here is the trace from a real run of this bundle, lightly cleaned (internal `platform.*` log
lines removed, IDs shortened) and annotated. It is the whole flow on one screen, the abstract
pieces made concrete:

```text
swarm runtime ready   contracts=./ticket-flow nodes=1 agents=2 events=6 listener=:8081
run started: run_id=82a6350e… status=running

trace ticket.created    delivered     subscriber=node/__runtime_replay_scope__   # 1
trace ticket.created    in_progress   subscriber=agent/classifier-agent          # 2
trace platform.agent_started          subscriber=node/__runtime_replay_scope__
trace ticket.classified delivered     subscriber=node/__runtime_replay_scope__   # 3
trace ticket.assigned   in_progress   subscriber=agent/resolver-agent            # 4
trace ticket.resolved   delivered     subscriber=node/__runtime_replay_scope__   # 5
trace ticket.resolution_confirmed     subscriber=node/__runtime_replay_scope__   # 6
```

The `subscriber=node/__runtime_replay_scope__` you see on most lines is not your
...
1. **`ticket.created` is published and persisted.** The event is written to the log before
   anything runs, so the run can be replayed from here. The orchestrator (the one system node)
   handles it in a single transaction: it mints the ticket and advances it to `new`. No
   separate "state changed" line appears because the transition *is* the handler, committed
   atomically.
2. **Same event, delivered to the classifier agent.** Routing came only from subscriptions:
   both the orchestrator and `classifier-agent` subscribe to `ticket.created`, so both receive
   it. `in_progress` means the agent's LLM turn is running.
3. **The classifier emits `ticket.classified`.** The orchestrator handles it, writes `category`
   and `priority` onto the ticket, advances to `assigned`, and emits `ticket.assigned`. One
   event in, one transition, one event out.
4. **`ticket.assigned` is delivered to the resolver.** It carries the `category` the
   orchestrator projected with `emit.fields`, which is all the resolver needs to decide.
5. **The resolver emits `ticket.resolved`.** (This was a `technical` ticket, so the resolver
   resolved it; a `billing` or `account` ticket would emit `ticket.escalated` here, and the
   guard you wrote would cap the retries.) The orchestrator saves the resolution and advances to
   `resolved`.
6. **`ticket.resolution_confirmed` closes the loop.** Its handler advances the ticket to
   `closed`, a terminal state, and the run goes quiet.

Every line is an event moving between the deterministic orchestrator and the LLM agents,
exactly the [sequence diagram](#the-mental-model) at the top, now with real timestamps behind
it. The agents only ever *emitted*; a system node decided what each event changed and what
fired next.

<Note>
  Want to see the other branch? Send a ticket the classifier will label `billing` or `account`
  ...
  the deterministic guard decides when to stop.
</Note>
