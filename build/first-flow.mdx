...
<Note>
  ...
  is configured: a real provider (`SWARM_LLM_RUNTIME_MODE=api` with `ANTHROPIC_API_KEY`) or the
  Claude CLI runtime (`SWARM_LLM_RUNTIME_MODE=cli_test`). The deterministic system-node steps
