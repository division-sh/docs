...
external
   ...
   |  ticket.created
   v
[orchestrator] create the ticket, state -> new       [classifier] reads it,
                                                       emits ticket.classified
   |
   |  ticket.classified
   v
[orchestrator] save category + priority, state -> assigned, emit ticket.assigned
   |
   |  ticket.assigned
   v
[resolver] emits ticket.resolved   OR   ticket.escalated
   |                                          |
   |  ticket.resolved                         |  ticket.escalated
   v                                          v
[orchestrator] save resolution,        [orchestrator] guard: under the limit?
 state -> resolved,                      count + 1, state -> assigned
 emit ticket.resolution_confirmed        (loops back to the resolver)
   |
   |  ticket.resolution_confirmed
   v
[orchestrator] state -> closed   (terminal)
```
