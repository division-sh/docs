...
A single LLM with a script is fine for a single task. The moment you want **many agents
collaborating on something large and long-running**, two things break a script: the model's
context fills up and it loses the plot, and the state you manage by hand drifts into
half-applied, contradictory messes. Swarm is built for that regime. Here is what the contracts
buy, with the flow you just wrote as the small example:
...
- **The state never goes weird.** The thinking is done by LLMs, but the truth is owned by
  deterministic system nodes. Every transition (guard, write, advance, emit) commits as one
  atomic, rule-checked step, so a ticket is always in exactly one declared state with
  consistent fields: no half-applied writes, no two writers racing, no "how did it get into
  *this* state?" An LLM steering its own control flow gives you no such guarantee.
