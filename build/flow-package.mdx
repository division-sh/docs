...
agents means no `agents.yaml` or `prompts/`, no handlers means no `nodes.yaml`, no events means
no `events.yaml`. A flow with only system nodes and no agents is valid.
