...
- **Wait for several events before acting** with `accumulate` (completion conditions,
  deduplication, and copying the gathered items to a typed entity field). See
  [Accumulation and projection](/reference/accumulation), including how `dedup_by` defaults.
