...
An agent is declared as a map entry in `agents.yaml`. The key is the agent's id (its address)
...
```yaml
resolver-agent:                       # map key = agent id = role (the default)
  model_tier: sonnet
...
| `role` | Optional override; defaults to the map key. Set it when the prompt file is named differently from the id, so the runtime loads `prompts/<role>.md` instead of `prompts/<map-key>.md`. |
