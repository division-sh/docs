...
| `session_scope` | `global`, `flow`, or `entity` — required for non-`task` modes. |
| `manager_fallback` | The escalation/hierarchy parent. Not a messaging grant. |
...
`native_tools` field. See [Tools](/build/tools).
...
(`current_date`, `agent_id`, `flow_instance_path`). Substitution is plain string replacement
— no logic — and a variable that resolves nowhere is left as-is.
