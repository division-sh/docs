...
Beyond declaring children, composition is mostly the **round trip**: getting work into a child
and getting a result back out to the right place. This page works one example end to end. The parent is an `orders` flow; it has a `notify` child (one instance, wired at
...
A **scoped subscription** qualifies the event with the producer flow's path, so the receiver
takes that one flow's output and nothing else:

```yaml
# subscribe to mailbox.review_requested, but only the copy the validation flow emits
subscribes_to:
  - validation/mailbox.review_requested
```

The form is `<producer-flow>/<event>`. For a template child with many instances, the
instance-wildcard form `<flow>/*/<event>` (below) addresses them all.
...
The child has its own state and its own entity; when it finishes, it sends a result back as an
**output pin** event:
