...
This is the hard half, and the one the old version of this page skipped. The child has its own
state and its own entity; when it finishes, it sends a result back as an **output pin** event:
...
the right order**, because the event arrived at the `orders` flow, not at a specific order
entity. It does that with `select_entity`, using the key the child carried in the payload:
