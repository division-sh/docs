...
<Note>
  The persistence and reconstruction plumbing for run-level fork is in place. The operator
  command surface for it is gated — the top-level `swarm fork` command is **retired**, and a
  dedicated run-fork control surface is the supported path going forward. For replaying
  individual events today, use the event/agent replay primitives above. See
