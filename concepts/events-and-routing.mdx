...
Every event has a typed payload schema in `events.yaml`. Payloads are **producer-complete**:
the emitting site must populate every declared field through `emit.fields` (or an agent's
emit tool). There are no payload defaults and no implicit passthrough from the triggering
event.
